...
<div class="py-12">
    <div class="max-w-7x1 mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-x1 sm:rounded-lg">
            <div class="p-2">
                <form method="POST" action="{{ route('spremi_auto')}}">
                    @csrf
                    <div>
                        <x-jet-label for="ime" value="{{ __('Ime') }}" />
                        <x-jet-input id="ime" class="block mt-1 w-full" type="text" name="ime" :value="old('ime')" required autofocus />    
                    </div>
                    <div class="mt-4">
                    <x-jet-label for="dat_proiz" value="{{ __('Datum proizvodnje') }}" />
                    <x-jet-input id="dat_proiz" class="block mt-1 w-full" type="date" name="dat_proiz" :value="old('dat_proiz')" required autofocus />
                    </div>
                    <div class="mt-4">
